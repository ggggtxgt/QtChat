...
MysqlConnPool::MysqlConnPool(const std::string &url, const std::string &user,
        ...
        : url_(url), user_(user), pass_(pass), schema_(schema), size_(size), b_stop_(false) {
    try {
        ...
        std::cout << "mysql pool init failed is: " << exception.what() << std::endl;
    }
}
...
void MysqlConnPool::checkConnection() {
    ...
    for (int i = 0; i < size; i++) {
        ...
        try {
            ...
            std::cout << "Error keeping connection alive: " << exception.what() << std::endl;
            // 重新创建连接并替换旧的连接
            sql::mysql::MySQL_Driver *driver = sql::mysql::get_mysql_driver_instance();
...
std::unique_ptr<SqlConnection> MysqlConnPool::getConnection() {
...
void MysqlConnPool::returnConnection(std::unique_ptr<SqlConnection> con) {
    std::unique_lock<std::mutex> lock(mutex_);
    if (b_stop_) {
        return;
    }
